...
<?php if ($tenderSection): ?><section class="tenders-section" id="tenders">
    <aside class="tender-intro">
        ...
        <?php if ($hasTenderAccess): ?><a href="<?= htmlspecialchars(url('/customer/dashboard#tenders')) ?>"><?= htmlspecialchars($tenderSection['button_label'] ?? 'View All Tenders') ?> <span>→</span></a><?php else: ?><button class="tender-access-trigger" type="button" data-tender-access-open><?= htmlspecialchars($tenderSection['button_label'] ?? 'View All Tenders') ?> <span>→</span></button><?php endif; ?>
    ...
    <div class="tender-content" id="tender-list">
        ...
        <?php if ($hasTenderAccess): ?><a class="tender-all-button" href="<?= htmlspecialchars(url('/customer/dashboard#tenders')) ?>">View All Tenders &amp; Notices <span>→</span></a><?php else: ?><button class="tender-all-button tender-access-trigger" type="button" data-tender-access-open>View All Tenders &amp; Notices <span>→</span></button><?php endif; ?>
    </div>
</section><?php endif; ?>

<?php if ($tenderSection && !$hasTenderAccess): ?><div class="tender-access-modal" id="tender-access-modal" aria-hidden="true"><div class="tender-access-backdrop" data-tender-access-close></div><section class="tender-access-dialog" role="dialog" aria-modal="true" aria-labelledby="tender-access-title"><button class="tender-access-close" type="button" data-tender-access-close aria-label="Close"><span></span><span></span></button><div class="tender-access-icon">✦</div><span class="tender-access-kicker">Partner-only opportunity desk</span><h2 id="tender-access-title">Unlock the full tender brief.</h2><p>Join Udyam Ventures as a Partner to view complete notices, tender documents, eligibility details and submission timelines.</p><div class="tender-access-steps"><span><b>1</b> Sign in or create your Partner Portal account</span><span><b>2</b> Choose a subscription plan that fits your organisation</span><span><b>3</b> Access every curated opportunity in one place</span></div><a class="tender-access-portal" href="<?= htmlspecialchars(url($header['portal_url'] ?? '/customer/login')) ?>" target="_blank" rel="noopener">Open Partner Portal <strong>↗</strong></a><small>New to Udyam? You can sign up from the Partner Portal.</small></section></div><script>(()=>{const modal=document.getElementById('tender-access-modal');if(!modal||modal.dataset.ready==='true')return;modal.dataset.ready='true';let last=null;const set=open=>{modal.classList.toggle('is-open',open);modal.setAttribute('aria-hidden',open?'false':'true');document.body.classList.toggle('tender-access-open',open);if(open){last=document.activeElement;setTimeout(()=>modal.querySelector('.tender-access-portal')?.focus(),100)}else last?.focus?.()};document.querySelectorAll('[data-tender-access-open]').forEach(button=>button.addEventListener('click',()=>set(true)));modal.querySelectorAll('[data-tender-access-close]').forEach(button=>button.addEventListener('click',()=>set(false)));window.addEventListener('keydown',event=>{if(event.key==='Escape'&&modal.classList.contains('is-open'))set(false)});})();</script><?php endif; ?>
