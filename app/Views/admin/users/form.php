...
<div class="form-group"><label>User Type</label><?php if($edit&&($values['user_type']??'')==='staff'):?><input type="hidden" name="user_type" value="staff"><input class="form-control" value="Staff" disabled><?php else:?><select class="form-control" name="user_type"><option value="admin" <?=($values['user_type']??'')==='admin'?'selected':''?>>Administrator</option><option value="customer" <?=($values['user_type']??'')==='customer'?'selected':''?>>Customer / Partner</option></select><?php endif?><small>Create complete employee profiles from Staff Management; this screen controls the shared login identity.</small><?php if(isset($errors['user_type'][0])):?><div class="invalid-feedback d-block"><?=htmlspecialchars($errors['user_type'][0])?></div><?php endif?></div>
